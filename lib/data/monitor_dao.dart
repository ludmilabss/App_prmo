...
class MonitorDao{
  ...
  Future<void> AtualizarSenha({required String novopassword, required String email, required String password}) async{
    DBHelper db1 = DBHelper();
    Database db = await db1.initDB();

    String sql = "UPDATE MONITOR SET password = ? WHERE email = ? AND password = ?;";
    final result = db.rawUpdate(sql, [novopassword, email, password]);
