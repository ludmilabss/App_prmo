...
class SharedPrefs{
  ...
  //usado para recuperar um conjunto de instâncias de uma classe espcífica, como objetos
  getInstance(){
    return SharedPreferences.getInstance();
  }
  //método para verifcar e atribuir true quando estiver logado
  login() async{
    SharedPreferences sharedP = await getInstance();
    await sharedP.setBool('MONITOR', true);
  }

//método para verifcar e atribuir false quando estiver deslogado
  logout() async {
    SharedPreferences sharedP = await getInstance();
